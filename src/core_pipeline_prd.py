...
FUNDER_SPECS: Dict[str, FunderSpec] = {
    "NIH": FunderSpec(
        key="NIH",
        template_md=Path("data_prd/inputs/dmp-template.md"),
        docx_template=Path("data_prd/inputs/nih-dms-plan-template.docx"),
...
class DMPPipeline:
    def __init__(self, config_path: str = "config/config.yaml", force_rebuild_index: bool = False):
        try:
            ...
            self.output_debug = self.config.resolve_path("data_prd/outputs/debug")
